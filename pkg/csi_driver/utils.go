...
func checkGcsFuseErr(_ bool, _ *corev1.Pod, targetPath string) (codes.Code, error) {
	klog.Infof("checkGcsFuseErr called")
	code := codes.Internal
	...
	klog.Infof("checkGcsFuseErr read file %s", emptyDirBasePath+"/error")
...
func checkSidecarContainerErr(isInitContainer bool, pod *corev1.Pod) (codes.Code, error) {
	klog.Infof("checkSidecarContainerErr called")
	code := codes.Internal
...
func PutFlagsFromDriverToTargetPath(flagMap map[string]string, targetPath string, fileName string) error {
	...
	// This file is rewritten if it already exists.
